...
**Code Review Agent**: In another project, I built an agent that helps with code reviews. Here's how both approaches played out:

- **Compaction in action**: When reviewing large pull requests, the agent summarizes previous review comments and focuses only on new changes, preventing context overload.
- **Evolving context**: Over time, the agent learned our team's coding standards and common issues, building a knowledge base that made reviews more consistent and insightful.

**The Hackathon Experience**: During a recent hackathon, I experienced firsthand why context engineering matters. Initially, I was asking Cursor to do everything at once—scaffold the project, implement features, handle edge cases. The results were all over the place.

Then I applied the principles: I broke down tasks into smaller, focused prompts (Anthropic's approach) while maintaining a running document of architectural decisions and learnings (ACE's approach). The difference was night and day. Instead of fighting with the AI, I was collaborating with it.
